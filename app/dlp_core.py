...
def extract_valid_phones_strict(text):
    ...
    for m in phonenumbers.PhoneNumberMatcher(text, DEFAULT_REGION):
        ...
        if phonenumbers.region_code_for_number(num) not in ALLOWED_REGIONS: continue
        if phonenumbers.number_type(num) not in ALLOWED_TYPES: continue
        nd=str(num.national_number)
        if not (8<=len(nd)<=12) or _repetitive(nd): continue
        raw=m.raw_string.strip()
        digits=re.sub(r"\D+","",raw)
        pk_local = (digits.startswith("03") and len(digits)==11)
        if not raw.startswith("+") and not (pk_local or _near_ctx(text,m.start,m.end)): continue
...
def score_text(text: str):
    ...
    if phones:
        ...
        return {"class":"phone","score":1.0,"policy":action,"phones":phones,
                "email_flag":email_flag,"cnic_flag":cnic_flag}

    proba = model.predict_proba(X_input)[0]
    for i,c in enumerate(classes):
        if _is_phone_label(c): proba[i]=0.0
    ...
    return {"class":top_cls,"score":round(sens_score,3),"policy":action,"phones":phones,
            "email_flag":email_flag,"cnic_flag":cnic_flag}
